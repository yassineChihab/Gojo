...
class AuthViewModel(
    ...
    fun postGoogleLogin(first_name: String, last_name: String, email: String, id_lang: Int) =
        viewModelScope.launch {
    ...
    fun sendOTP(phoneNumber: String, code:String) = viewModelScope.launch {
        _otpResponse.postValue(Resource.Loading())
        _otpResponse.postValue(repository.sendOTP(phoneNumber,code))
    }

    fun verifyOTP(phoneNumberArg: String, isocode:String ,code: Int) = viewModelScope.launch {
        _verificationOtpResponse.postValue(Resource.Loading())
        _verificationOtpResponse.postValue(repository.verifyOTP(phoneNumberArg,isocode ,code))
    }
